 package ca.pfv.spmf.algorithms.frequentpatterns.fpgrowth_diverse;
 ...
 */
public class AlgoFPGrowthDiverse 
{
	 ...
	 */
	public Itemsets runAlgorithm(double minsupp, double minDiv, int height_of_hierarchy,String input, String output,int total_branches) throws FileNotFoundException, IOException {
		// record start time
		...
		///// ************************************* IMPROVE***********************
		Map<String,String> encode = Encode("ca/pfv/spmf/input/diverse_data/hierarchy.txt");
...
//			Set<Integer> alreadySeen = new HashSet<Integer>();
		...
		//System.out.println("FP tree = "+tree.toString());
		...
		if(tree.headerList.size() > 0) {
			...
			fpgrowth(tree, encode,itemsetBuffer, 0, transactionCount, mapSupport,plf,minDiv,total_branches,height_of_hierarchy);
		}
	 ...
	 */
	private void fpgrowth(FPTree tree, Map<String,String> encode,int [] prefix, int prefixLength, int prefixSupport, Map<Integer, Integer> mapSupport,float [] plf,double minDiv, int tot_branches,int height_of_hierarchy) throws IOException {
////		======= DEBUG ========
...
//		System.out.println(tree);
		...
		{ // Case 2: the FPtree does not contain single path
			...
			for(int i = tree.headerList.size()-1; i>=0; i--)
			{
				...
				//System.out.println("current beta= ");
                ...
                ///    FIND DR AND MAXDR FOR CURRENT BETA
				
				
				/*
				System.out.println("the entire encode array is : ");
				...
				*/
				...
				String [] encodedprefix= new String[prefixLength+1];
				//System.out.println("prefixlength= "+prefixLength);
			    
				for (int z = 0; z < (prefixLength+1); z++) 
				{
					//System.out.println("entered loop");
					encodedprefix[z]=encode.get(Integer.toString(prefix[z]));
				}
				
				//System.out.println("after loop");
				
                List<String> list =Arrays.asList(encodedprefix);
                
                //System.out.println("after list staelment");
				
                //System.out.println("prefix[0]= "+prefix[0]+ "encodedprefix[0]="+encodedprefix[0]);
				
                //System.out.println("finddivrank is being called with the enocded set= "+String.join(" ", list));
				
				ArrayList<Double> DRarr=FindDivRank(encodedprefix,plf,height_of_hierarchy,minDiv,tot_branches); 
				double DivRank=DRarr.get(0);
				double maxDivRank=DRarr.get(1);
				...
				if(DivRank>=minDiv)
				{
					...
					//saveItemsetEncoded(prefix);
					
				}
				
				//System.out.println("maxdivrank= "+maxDivRank+"))))))))))))))))))))))))))))))))))))))))))))))");
				if(maxDivRank<minDiv)
				{
...
1/
				...
				if(treeBeta.root.childs.size() > 0)
				{
					...
					fpgrowth(treeBeta,encode, prefix, prefixLength+1, betaSupport, mapSupportBeta,plf,minDiv,tot_branches,height_of_hierarchy);
	 ...
	 */
	private void saveAllCombinationsOfPrefixPath(FPNode[] fpNodeTempBuffer, int position, int[] prefix, int prefixLength) throws IOException 
	{
	/////// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1 CONDITION ADDED !!!!!!!!!!!!!!!!
		
		//System.out.println("prefix in saveallcombo fn:   "+ String.join(" ", (Arrays.asList(prefix)).toString()));
	...
	private ArrayList<Double> FindDivRank(String[] efp,float[] plf, int h,double minDiv,int tot_branches)
	{
		...
		//List<String> list =Arrays.asList(efp);
		//System.out.println("num_branches is being called with encoded set= "+ String.join(" ", list));
	...
	private int num_branches(String[] arr) 
	{
		...
		//System.out.println(" inside num_branches : temp= "+ temp);
		//System.out.println("   ++++++++++++++++++++++++++++++  ");
		//System.out.println(" inside num_branches : temp= "+ temp);
